...
// Your task is to find a way to merge them into a single one, sorted in asc order. Complete the function
...
//   const filteredArr = combinedArray.filter((value, index, self) => {
//   return self.indexOf(value) === index;
// });

    for (let i = 0; i < combinedArray.length; i++ ) {
        if(combinedArray[i] !== combinedArray[i + 1] ) {
            filteredArr.push(combinedArray[i])
        }
    }
